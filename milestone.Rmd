...
library(xtable)
if (!exists("ds.stat")) {
    ds.stat <- get_dataset_stats("Coursera-SwiftKey/final")
    ds.stat$file <- gsub("Coursera-Swiftkey/final/.*/(..)_..(.*).txt", "\\1\\2",
                         as.character(ds.stat$file), perl=TRUE, ignore.case=TRUE)
    ds.stat[, c(2,3,4,6)] <- round(ds.stat[, c(2,3,4,6)] / 1000000, digits = 4)
    names(ds.stat) <- c("File", "Lines (mil)", "Words (mil)",
...
xt <- xtable(data.frame(Type=c("Training %", "CV %", "Test %"),
                        Project=c(60,20,20), Report=c(5, NA, NA)))
print(xt, type='html')
```
...
if (!exists("ds.prep.stat")) {
    ds.prep.stat <- get_dataset_stats("data.05")
    ds.prep.stat$file <- gsub("data.05/.*/(..)_..(.*).txt.(.*)", "\\1\\2 \\3",
                              as.character(ds.prep.stat$file), 
                              perl=TRUE, ignore.case=TRUE)
    ds.prep.stat[, c(2,3,4,6)] <- round(ds.prep.stat[, c(2,3,4,6)] / 1000000, digits = 4)
    names(ds.prep.stat) <- c("File", "Lines (mil)", "Words (mil)", 
                         "Unique (mil)", "% Unique", "Bytes (MB)")    
    ds.prep.stat <- ds.prep.stat[grep("^en", ds.prep.stat$File),]
}
```
```{r echo=FALSE,results='asis'}
...
freq.en.blogs.3grams <- freq_ngrams(en.blogs.3grams, 25, "Most Frequent EN Blogs 3-Grams")
...
#freq.en.blogs.1grams; freq.en.blogs.2grams; freq.en.blogs.3grams
